...
function Settings() {
  ...
  const [vendorData, setVendorData] = useState({ name: '', description: '', mobile: '', image: '' });
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (profileData.p_image && profileData.p_image !== res.data.image && profileData.p_image instanceof File) {
      const uploaded = await uploadFile({
        file: profileData.p_image,
        keyPrefix: `profiles/${userData?.user_id}`,
      });
      if (uploaded?.key) {
  ...
  const handleShopFormSubmit = async (e) => {
    ...
    const formData = new FormData();
    if (vendorData.image && vendorData.image !== res.data.image && vendorData.image instanceof File) {
      const uploaded = await uploadFile({
        file: vendorData.image,
        keyPrefix: `vendors/${userData?.vendor_id}`,
      });
      if (uploaded?.key) {
        formData.append('image', uploaded.key);
      }
