  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.entries(product)?.forEach(([key, value]) => {
        console.log(`key: ${key} - value: ${value}`);
        if (key === 'image') {
          // Upload first; do not send raw file
          return;
        }
        formData.append(key, value);
      });
      ...
      if (product.image && product.image instanceof File) {
        ...
        if (uploaded?.key) {
          formData.append('image', uploaded.key);
        }
      ...
      colors?.forEach((color, index) => {
        Object.entries(color).forEach(([key, value]) => {
          if (key === 'image' && value && value.file && value.file.type.startsWith('image/')) {
            // Upload first; send only storage key
          } else {
      ...
      for (let index = 0; index < colors.length; index++) {
        ...
        if (c?.image?.file) {
          const uploaded = await uploadFile({
            file: c.image.file,
            keyPrefix: `products/${userData?.vendor_id}/colors`,
          });
          if (uploaded?.key) {
            formData.append(`colors[${index}][image]`, uploaded.key);
          }
        } else if (typeof c?.image === 'string' && c.image.length > 0) {
          formData.append(`colors[${index}][image]`, c.image);
        }
      }
      ...
      for (let index = 0; index < gallery.length; index++) {
        ...
        if (g?.image?.file) {
          const uploaded = await uploadFile({
            file: g.image.file,
            keyPrefix: `products/${userData?.vendor_id}/gallery`,
          });
          if (uploaded?.key) {
            formData.append(`gallery[${index}][image]`, uploaded.key);
          }
        } else if (typeof g?.image === 'string' && g.image.length > 0) {
          formData.append(`gallery[${index}][image]`, g.image);
        }
      }
