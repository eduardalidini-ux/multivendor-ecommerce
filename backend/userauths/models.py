...
def user_directory_path(instance, filename):
    ...
    model_folder = getattr(instance, "_meta", None) and instance._meta.model_name or "files"
    key_prefix = f"user_{user.id}/{model_folder}" if user else f"user_file/{model_folder}"

    base = os.path.basename(filename or "file")
    name, ext = os.path.splitext(base)
    ext = (ext.lstrip(".") or "bin").lower()

    safe_name = slugify(name) or "file"
    unique = uuid.uuid4().hex[:8]
    final_name = f"{safe_name}__{unique}.{ext}"

    return f"{key_prefix}/{final_name}"
...
class Profile(models.Model):
    ...
    def thumbnail(self):
        return mark_safe('<img src="%s" width="50" height="50" object-fit:"cover" style="border-radius: 30px; object-fit: cover;" />' % (self.image.url))
